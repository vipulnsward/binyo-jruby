...
# TODO: update
MANIFEST = FileList["Rakefile", "Manifest.txt", "README.rdoc", "License.txt", "lib/binyo-jruby.jar", "lib/**/*", "spec/**/*"]
File.open("Manifest.txt", "w") {|f| MANIFEST.each {|n| f.puts n } }

task :default => [:build, :spec]

file "lib/binyo-jruby.jar" => :build

desc "Delete artifact files"
task :clean do
  rm_f FileList['lib/binyo-jruby.jar']
  ant :clean
end
